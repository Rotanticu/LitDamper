...
namespace LitDamper
{
    internal class DamperBuilderBuffer<TValue, TOptions>
        where TValue : unmanaged
        where TOptions : unmanaged, IDamperOptions
    {
        static DamperBuilderBuffer<TValue, TOptions> PoolRoot = new();

        public static DamperBuilderBuffer<TValue, TOptions> Rent()
        {
            DamperBuilderBuffer<TValue, TOptions> result;
            if (PoolRoot == null)
            {
                result = new();
            }
        ...
        public static void Return(DamperBuilderBuffer<TValue, TOptions> buffer)
        {
            buffer.Version++;
            buffer.Data = DamperData<TValue, TOptions>.Default;
            buffer.CallbackData = DamperCallbackData.Default;
        ...
        public DamperBuilderBuffer<TValue, TOptions> NextNode;
        public bool IsPreserved;
        public bool BindOnSchedule;

        public DamperData<TValue, TOptions> Data = DamperData<TValue, TOptions>.Default;
        public DamperCallbackData CallbackData = DamperCallbackData.Default;
        public AnimationCurve AnimationCurve;

        public IDamperScheduler Scheduler;
    }
    ...
    public struct DamperBuilder<TValue, TOptions, TAdapter> : IDisposable
        where TValue : unmanaged
        where TOptions : unmanaged, IDamperOptions
        where TAdapter : unmanaged, IDamperAdapter<TValue, TOptions>
    {
        internal DamperBuilder(DamperBuilderBuffer<TValue, TOptions> buffer)
        {
            this.buffer = buffer;
        ...
        internal DamperBuilderBuffer<TValue, TOptions> buffer;
        ...
        public readonly DamperBuilder<TValue, TOptions, TAdapter> WithOptions(TOptions options)
        {
            CheckBuffer();
        ...
        /// <returns>This builder to allow chaining multiple method calls.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public readonly DamperBuilder<TValue, TOptions, TAdapter> WithOnCancel(Action callback)
        {
            CheckBuffer();
        ...
        /// <returns>This builder to allow chaining multiple method calls.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public readonly DamperBuilder<TValue, TOptions, TAdapter> WithOnComplete(Action callback)
        {
            CheckBuffer();
        ...
        /// <returns>This builder to allow chaining multiple method calls.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public readonly DamperBuilder<TValue, TOptions, TAdapter> WithCancelOnError(bool cancelOnError = true)
        {
            CheckBuffer();
        ...
        /// <returns>This builder to allow chaining multiple method calls.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public readonly DamperBuilder<TValue, TOptions, TAdapter> WithBindOnSchedule(bool bindOnSchedule = true)
        {
            CheckBuffer();
        ...
        /// <returns>This builder to allow chaining multiple method calls.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public readonly DamperBuilder<TValue, TOptions, TAdapter> WithScheduler(IDamperScheduler scheduler)
        {
            CheckBuffer();
        ...
        public DamperHandle RunWithoutBinding()
        {
            CheckBuffer();
            SetMotionData();
        ...
        /// <returns>Handle of the created motion data.</returns>
        public DamperHandle Bind(Action<TValue> action)
        {
            CheckBuffer();
            SetMotionData();
        ...
        /// <param name="action">Action that handles binding</param>
        /// <returns>Handle of the created motion data.</returns>
        public DamperHandle BindWithState<TState>(TState state, Action<TValue, TState> action) where TState : class
        {
            CheckBuffer();
            SetMotionData();
        ...
        /// <param name="action">Action that handles binding</param>
        /// <returns>Handle of the created motion data.</returns>
        public DamperHandle BindWithState<TState1, TState2>(TState1 state1, TState2 state2, Action<TValue, TState1, TState2> action)
            where TState1 : class
            where TState2 : class
        {
            CheckBuffer();
        ...
        /// <param name="action">Action that handles binding</param>
        /// <returns>Handle of the created motion data.</returns>
        public DamperHandle BindWithState<TState1, TState2, TState3>(TState1 state1, TState2 state2, TState3 state3, Action<TValue, TState1, TState2, TState3> action)
            where TState1 : class
            where TState2 : class
            where TState3 : class
        {
        ...
        /// <returns>This builder to allow chaining multiple method calls.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public readonly DamperBuilder<TValue, TOptions, TAdapter> Preserve()
        {
            CheckBuffer();
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal DamperHandle Schedule(IDamperScheduler scheduler, ref DamperData<TValue, TOptions> data, ref DamperCallbackData callbackData)
        {
            ...
            if (scheduler == null)
            {
                ...
                {
                    handle = DamperDispatcher.Schedule<TValue, TOptions, TAdapter>(data, callbackData, PlayerLoopTiming.Update);
                }
                else
                {
            ...
            if (DamperTracker.EnableTracking)
            {
                DamperTracker.AddTracking(handle, scheduler);
            }
        ...
        public void Dispose()
        {
            ...
            DamperBuilderBuffer<TValue, TOptions>.Return(buffer);
            buffer = null;
        }
        ...
        {
            buffer.Data.Core.Status = DamperStatus.Scheduled;
        }
